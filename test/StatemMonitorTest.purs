...
type Internal = Void
type TimerName = Void
data TimerContent = Void

type OurType = StatemType Info Internal TimerName TimerContent Data StateId State

newtype OurPid = OurPid (StatemPid Info Internal TimerName TimerContent Data StateId State)
...
startLink valueServerPid = do
  ...
  where
    init = do
      ...
      let initialData = { monitorRef, downReceived: false }
...
downReceived =
  ...
  where
    impl from _state { downReceived } = do
      let actions = Statem.newActions # Statem.addReply (Statem.mkReply from downReceived)
