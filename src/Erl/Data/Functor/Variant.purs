module Erl.Data.Functor.Variant
  ( VariantF
  , inj
  , prj
  , on
...
import Erl.Data.Variant.Internal (class Contractable, class VariantFMatchCases) as Exports
import Erl.Data.Variant.Internal (class Contractable, class VariantFMatchCases, class VariantTags, VariantFCase, VariantCase, contractWith, lookup, unsafeGet, unsafeHas, variantTags)
...
class
  FoldableVFRL rl row <= TraversableVFRL rl row | rl -> row where
...
inj ∷
  ...
  f a →
  VariantF r2 a
inj p value = coerceV $ VariantFRep { type: toAtom $ (atom :: Atom sym), value, map }
  where
...
on p f g r =
  case coerceY r of
    VariantFRep v | v.type == (toAtom $ (atom :: Atom sym)) → f v.value
    _ → g (coerceR r)
  where
...
case_ r =
  unsafeCrashWith case unsafeCoerce r of
    VariantFRep v → "Data.Functor.Variant: pattern match failure [" <> show v.type <> "]"
...
instance showVariantF ∷ (RL.RowToList r rl, VariantTags rl, VariantFShows rl a, Show a) ⇒ Show (VariantF r a) where
  show v1 =
    let
      ...
      body = lookup (ErlAtom.atom "show") v.type tags shows (unsafeCoerce v.value ∷ VariantCase)
    in
