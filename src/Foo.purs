...
import Bar as Bar
import Erl.Data.Functor.Variant (VariantF)
...
processMParse :: forall msg r. Foreign -> Run ((PROCESSM msg) + r) (Maybe msg)
...
monitor pid = do
  Run.lift _monitor (Monitor pid identity)
...
class ProcessHandler stack acc where
  handle :: {| acc} -> VariantF stack ({| acc} -> (Effect {| acc})) -> Effect {| acc}
...
class ProcessHandlerRL stackL stack acc | stackL -> stack acc where
  handleL :: Proxy stackL -> {| acc} -> VariantF stack ({| acc} -> (Effect {| acc})) -> Effect {| acc}
...
instance ProcessHandlerRL RL.Nil () acc where
  ...
  handleL _ acc variant = do
    let
      fn :: functorType ({| acc} -> Effect {| acc}) -> Effect {| acc}
      fn functorInstance = do
        handleFunctor acc (Record.get (Proxy :: _ sym)) (\val -> Record.set (Proxy :: _ sym) val acc) functorInstance

      recurse :: VariantF stackTail ({| acc} -> (Effect {| acc})) -> Effect {| acc}
...
class FunctorHandler functorType functorAcc | functorType -> functorAcc where
  handleFunctor :: forall acc. {| acc} -> ({| acc} -> functorAcc) -> (functorAcc -> {| acc}) -> functorType ({| acc} -> Effect {| acc}) -> Effect {| acc}

instance FunctorHandler Effect Unit where
  handleFunctor acc _ _ cb = do
    cb' <- cb
    cb' acc

instance FunctorHandler (ProcessM msg) Unit where
  handleFunctor acc _ _ (ProcessMParse cb) = do
    cb acc

instance FunctorHandler TrapExitF Unit where
  handleFunctor acc _ _ (TrapExitParse cb) = do
    cb acc

instance
  FunctorHandler MonitorF (Map MonitorRef Unit) where
  handleFunctor acc get set  (Monitor pid cb) = do
    ref <- Bar.monitorImpl pid
    cb ref $ set $ Map.insert ref unit (get acc)
  handleFunctor acc _ _ (Demonitor ref cb) = do
    let
      doTheDeMonitor :: MonitorRef -> Effect Unit
      doTheDeMonitor _ = pure unit
    doTheDeMonitor ref
    cb acc
  handleFunctor acc _ _ (MonitorParse cb) = do
    cb acc
...
handle_info :: forall r acc. ProcessDefault r acc => ProcessHandler r acc => Run r Unit -> Effect {| acc}
