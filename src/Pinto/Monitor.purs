...
foreign import startMonitor :: forall state msg. ServerPid state msg -> Effect MonitorRef
...
pid :: forall state msg. ServerPid state msg -> (MonitorMsg -> Effect Unit) -> Effect (MR.RouterRef MonitorRef)
pid p cb =
...
process :: forall state msg. ServerPid state msg -> (MonitorMsg -> Effect Unit) -> Effect (MR.RouterRef MonitorRef)
process p cb =
  MR.startRouter (startMonitor p) stopMonitor $ (\msg -> do
                                                      _ <- handleMonitorMessage Down cb msg
                                                      _ <- MR.stopRouterFromCallback
