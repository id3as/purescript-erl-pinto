...
newtype ResultT cont stop msg state result = ResultT (ReaderT (Context cont stop msg state) Effect result)
...
-- | a GenServer
type ServerInstance cont stop msg state = RegistryInstance (ServerPid cont stop msg state) (ServerType cont stop msg state)
...
newtype Context cont stop msg state = Context
  { handleInfo :: Maybe (InfoFn cont stop msg state)
  , handleContinue :: Maybe (ContinueFn cont stop msg state)
  , terminate :: Maybe (TerminateFn cont stop msg state)
  , trapExits :: Maybe (ExitMessage -> msg)
